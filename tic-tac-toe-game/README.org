...
* Python Implementation 
  ...
  Interesting the function we return to evaluate the move is a form of generator isn't it? That would make it easy to short circuit the first 3 calls. Interestingly, since you are receiving a coordinate each time, then maybe we should style it as the more "advanced" form of a generator which is sent values (in this case the coordinate). This is supported in python with the generator's ~send~ function but its a totally weird, anti-intuitive syntax where you basically have to call ~next~ once and *afterwards* call ~send~ to advance the iterator properly. Still, a good idea here, even if the python approach to it is bizarre.

  Of course that is all of little help if we have to keep the full list of coordinates we've seen in memory, but we actually don't. No one is asking us to track *where* each coordinate we've seen is placed, instead we are asked to track how many matches fall on each row, column, and diagonal and if any of those are more than 3 than good for us, this player has won.
  ...
  #+begin_src python
    ...
    def move_evaluator(player: _MoveEvaluatorPlayer) -> Callable[[Coordinate], Optional[_MoveEvaluatorPlayer]]:
        ...
        has_match = lambda: any(filter(lambda n : n>=3, chain(coordinates_in_row, coordinates_in_column, coordinates_on_diagonal)))
        ...
        def record(coordinate: Coordinate):
            ...
            logging.debug('logged', coordinate, 'tracked state', coordinates_in_row, coordinates_in_column, coordinates_on_diagonal)
        ...
        def evaluate() -> Generator[Optional[_MoveEvaluatorPlayer], Coordinate, None]:
            ...
            while True:
                coordinate = yield (player if has_match() else None)
                record(coordinate)
...
** Playground
  ...
  no..that's not it. We're getting false positives from player A. Lets check it in isolation
  ...
  #+begin_src python :eval yes
    ...
    print('not A', [move(c) for c in [[0,0], [1,1], [2,1]]])
  #+end_src
