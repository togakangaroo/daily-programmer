...
** Parser
   ...
   Lets say I'm tracking around the edges - that is I'm looking at vertex =[0 1]= and I need to decide which direction to step to next. There are 4 directions and the way we can pick is
    ...
    #+begin_src racket 
      ...
      (define (vertex-in-bounds? board vertex)
        (match-define (vector board-height board-width) (array-shape board))
        (match-define (vector row column) vertex)
        (and (<= 0 column (sub1 board-width))
             (<= 0 row (sub1 board-height))))
      
      (define (segment-on-perimeter? board vertex-1 vertex-2)
        (and  (vertex-in-bounds? board vertex-2)
              (vertex-in-bounds? board vertex-1)
              (not (equal? (array-ref board vertex-1)
                           (array-ref board vertex-2)))))
      
      (define (find-perimeter board initial-vertex)
        (let rec ([current-vertex initial-vertex]
                  [previously-visited (set)])
          (println (~a "currently" current-vertex))
          (let ([visited (set-add previously-visited current-vertex)])
               (unless (for*/or ([direction #[#[0 1] #[1 0] #[0 -1] #[-1 0]]])
                         (define next-vertex (vector-map + current-vertex direction))
                         (println (~a "considering" next-vertex))
                         (if (and (not (set-member? visited next-vertex))
                                  (segment-on-perimeter? board current-vertex next-vertex))
                             (rec next-vertex visited)
                             #f))
                 (println (~a "all done, visited:" visited))
                 (set-count visited)))))
    ...
    #+begin_src racket
      ...
      (define (indicies-of-cells-adjacent-to-move from to)
        (match (vector- to from)
          [(vector 0 1)  (list from (vector+ from up1))]
          [(vector 0 -1) (list to   (vector+ to up1))]
          [(vector 1 0)  (list from (vector+ from left1))]
          [(vector -1 0) (list to   (vector+ to left1))]))
...
** Playground
    ...
    #+RESULTS:
    : ob-racketwdjRct.rkt:3:1: vector<: unbound identifier
    :   in: vector<
    :   location...:
    :    ob-racketwdjRct.rkt:3:1
