...
** Parser
   ...
   #+begin_src racket 
     ...
     (define (cell-in-bounds? board cell-position)
       (match-define (vector max-row max-column) (vector-map sub1 (array-shape board)))
       (match-define (vector row column) cell-position)
       (and (<= 0 row max-row)
            (<= 0 column max-column)))
     
     
     (define (vertex-in-bounds? board vertex)
       (match-define (vector max-vertex-row max-vertex-column) (array-shape board))
       (match-define (vector row column) vertex)
       (and (<= 0 row max-vertex-column)
            (<= 0 column max-vertex-column)))
     ...
     (segment-on-perimeter? board #[2 1] #[2 2])
   #+end_src
