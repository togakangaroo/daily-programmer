...
Ok cool, now lets do another one. How about our disallowed constraint? That's nice and simple
...
#+begin_src python :tangle no :results output pp replace
  ...
  filters = (
      DisallowedConstraint(square, pos),
...
#+RESULTS:
: possibilities are [1, 2, 3, 4, 5, 6, 7, 8, 9] invoking <class '__main__.DisallowedConstraint'>
