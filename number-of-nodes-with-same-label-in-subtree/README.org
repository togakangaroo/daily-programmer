...
#+begin_src typescript
  ...
  const nodesNavigableFrom : Map<Index, {readonly label: Node, readonly navigableFrom: Set<Node>}> = new Map()
#+end_src
...
#+name: setCount
#+begin_src typescript
  const setCount = <T> (set: Set<T>, predicate: (_: T) => Boolean) => {
      let cnt = 0;
...
#+begin_src typescript :eval yes
  ...
  for(const [index, {node: {label}, navigableFrom}] of nodesNavigableFrom.entries()) {
      const sameLabelFactor = setCount(navigableFrom, (node) => node.label === label)
...
#+name: evaluate-lisp-string
#+begin_src emacs-lisp :var lisp-as-string="'((1 2) (3 4))" :results code
  (car (read-from-string lisp-as-string))
#+end_src

#+RESULTS: evaluate-lisp-string
#+begin_src emacs-lisp
'((1 2)
  (3 4))
#+end_src

#+begin_src emacs-lisp :var data=example-1 :wrap "src json"
  (-let* ((((_ _ labels)) data))
    (json-encode  `(labels ,labels)))
#+end_src
...
#+begin_src emacs-lisp :var data=example-1 :wrap "src json"
  (-let* ((((_ edges-lisp-string)) data)
          ((edges-list) (read-from-string edges-lisp-string))
          (edges (apply 'vector edges-list))

          (((_ _ labels)) data)

          (((_ _ _ output-lisp-string)) data)
          ((output-list) (read-from-string output-lisp-string))
          (output (apply 'vector output-list)))
    (json-encode `((edges ,edges)
                   (labels ,labels)
                   (output ,output))))
#+end_src
...
#+RESULTS:
#+begin_src json
{"edges":[[[0,1],[0,2],[1,4],[1,5],[2,3],[2,6]]],"labels":["abaedcd"],"output":[[2,1,1,1,1,1,1]]}
#+end_src



#+begin_src typescript :eval yes :var testData=example-1
...
#+begin_src emacs-lisp :eval yes :var value=example-1
  (json-encode '[[7 5])
