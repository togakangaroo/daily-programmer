...
***  Challenge 1
    ...
    #+begin_src racket :noweb strip-export :results output :exports both :eval no
      (require gregor)
      (require json)
      (require threading)
      (require racket/hash)
      ...
      (define order-day-of-the-week-name (lambda~> (hash-ref 'placedOn)
                                                   iso8601->date
                                                   date-day-name))
...
**** 10 day period with the most orders
     ...
     So this has the need to chunk orders by their `placedOn` date within 10 of each other. Lets create a generic funciton thats easier to think about we're going to take a sequence of ordered numbers and lets group them so that they are within 3 of each other
     ...
     #+begin_src racket :noweb strip-export :results output :exports both :var data=chunk-by-input
       ...
       (define (ducky x acc)
         (cons x acc))
       ;;(filter (curryr < (- x 3)) (cons x acc)))
       (transduce (first data)
                  (folding (lambda (a b) (cons b a)) '())
                  (mapping first)
                  #:into into-list)
     #+end_src
     ...
     A second step will filter this collection for only lists that are not proper supersets of the previous item

     #+begin_src racket :noweb strip-export :results output :exports both :var data=chunk-by-input
       (require racket/generator)
       (require racket/match)

       (define (fold-by transform-queue collection)
         (in-generator (let ([queue '()])
                         (for ([item collection])
                           (let-values ([(to-yield new-queue) (transform-queue item queue)])
                             (set! queue new-queue)
                             (yield (cons to-yield new-queue)))))))
       (define (ducky item collection)
         (partition (curryr < (- item 3))
                    collection))

       (sequence->list (fold-by ducky (first data)))
     #+end_src
     ...
     #+begin_src racket :noweb strip-export :results output :exports both
       ...
       (~>> (load-orders)
            (sort _ string<? #:key (lambda~> (hash-ref 'placedOn))))
     #+end_src


     #+begin_src racket :noweb strip-export :results output :exports both :var data=chunk-by-input
       (partition (curryr < (- 12 3))
                  (first data))
     #+end_src
