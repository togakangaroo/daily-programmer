...
**** 10 day period with the most orders
     ...
     #+begin_src racket :eval no
       (define (minimum-maximum less-than? lst)
         (let ([sorted-lst (sort lst less-than?)])
           (values (last sorted-lst) (first sorted-lst))))
     #+end_src
     ...
     #+begin_src racket :noweb strip-export :results output :exports both
       ...
       (struct date-stats (min-date max-date order-count) #:transparent)
       (define (date-stats-of-order-group orders)
         (let-values ([(min max) (~>> orders
                                      (map placed-on)
                                      (minimum-maximum string<?))])
           (date-stats min max (length orders))))

       (~>> (load-orders)
            (sort _ string<? #:key placed-on)
            (group-into-sliding-distance-windows orders-within-10-days?)
            (sort _ > #:key length)
            (group-by length)
            first
     ...
     #+RESULTS:
     : (list (date-stats "2019-09-11" "2019-09-01" 7) (date-stats "2020-03-16" "2020-03-08" 7))
